...
dist_metods <- dist_methods[-which(dist_methods == "ANY")]
...
for (j in dist_methods) {
  ...
  p <- NULL
  p <- plot_ordination(microSubRelFiltSpistFilt, iMDS)
...
ggplot(mothurPCOA, title='PCoA of Stylophora pistillata samples') +
...
spistSIMPROF <- simprof(spistShared, num.expected=10, num.simulated=99, method.cluster='average', method.distance='euclidean', method.transform='identity', alpha=0.05, sample.orientation='column', silent=FALSE)

...
Pretty hard to decide on a distance metric I think - most people use Bray Curtis so guess I'll use that even though but I'm pretty sure Pat Schloss (the mothur guy) doesn't like this measure. 
...
mothurNMDSplot + geom_polygon(aes(x=axis1, y=axis2, fill = site), alpha=0.5) + ggtitle("nMDS of Stylophora pistillata samples")
```
...
df_ell <- data.frame()
for(g in levels(mothurNMDSmeta$site)){
  df_ell <- rbind(df_ell, cbind(as.data.frame(with(mothurNMDSmeta[mothurNMDSmeta$site==g,], ellipse(cor(axis1, axis2), scale=c(sd(axis1), sd(axis2)), centre=c(mean(axis1), mean(axis2))))),site=g))
}

ggplot(data=mothurNMDSmeta, aes(x=axis1, y=axis2, color=site)) +
  geom_point(size=1.5, alpha=0.6) +
  geom_path(data=df_ell, aes(x=x, y=y, color=site), size=1, linetype=1)

```

euclidean and 'squareroot' had 21 groups, many with just a single sample
euclidean and 'identity' (no transformation) had 12 groups, 
braycurtis, 'identity', had 37 and some bigger groups
braycurtis, 'squareroot', had 43 clusters and not many bigger groups..
